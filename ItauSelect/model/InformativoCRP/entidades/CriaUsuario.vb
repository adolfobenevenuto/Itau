...
#Region "Member Variables"
  ...
  ' constantes para SQL
  Private Shared ReadOnly C_GET_LIST As String = "select id_usu as 'Id', inscricao as 'Inscricao', cpf as 'CPF', nome as 'Nome', logon as 'Logon', senha as 'Senha', telefone as 'telefone', email as 'Email' from tbl_usuario where Excluido = 0 order by nome"
  Private Shared ReadOnly C_GET_DATA As String = "select * from tbl_usuario where id_usu = @id"
  Private Shared ReadOnly C_INSERT As String = "insert into tbl_link (inscricao,cpf,nome,logon,senha,telefone,email,excluido,criador,dt_criacao,atualizador,dt_atualizacao) " + _
                                               ...
                                               "select * from tbl_usuario where id_usu = @@identity"
  Private Shared ReadOnly C_DELETE As String = "update tbl_usuario set excluido = 1 where id_usu = @id"
  Private Shared ReadOnly C_UPDATE As String = "update tbl_usuario set inscricao = @inscricao, cpf = @cpf, nome = @nome, logon = @logon, senha = @senha, telefone = @telefone, email = @email, dt_atualizacao = getdate() where id_usu = @id"
