...
#Region "Member Variables"
  ...
  Private Shared ReadOnly C_GET_LIST As String = "select id as 'Id', crp as 'Crp', cpf as 'CPF',nome as 'Nome'," + _
                                                  "cracha as 'cracha', email as 'Email', residencial as 'residencial'," + _
                                                  ...
                                                  "from tbl_congresso_deficiencia_mesa where Excluido = 0 order by nome"

  Private Shared ReadOnly C_GET_DATA_TOTAL_EVENTO As String = "select * from tbl_congresso_deficiencia_mesa where Excluido = 0 and congresso = {0} order by id desc"
  Private Shared ReadOnly C_GET_DATA_CPF As String = "select * from tbl_congresso_deficiencia_mesa where cpf = @cpf and Excluido = 0"
  Private Shared ReadOnly C_GET_DATA_CPF_MESA As String = "select * from tbl_congresso_deficiencia_mesa_mesa where cpf = @cpf and Excluido = 0"

  Private Shared ReadOnly C_GET_DATA_TOTAL As String = "select * from tbl_congresso_deficiencia_mesa where excluido = 0 order by id desc"
  Private Shared ReadOnly C_GET_DATA_ESTUDANTE As String = "Select count(*) as xestudante from tbl_congresso_deficiencia_mesa where titulacao = 'GRADUÇÃO INCOMPLETA'"

  Private Shared ReadOnly C_GET_DATA As String = "select * from tbl_congresso_deficiencia_mesa where id = @id"

  Private Shared ReadOnly C_INSERT As String = "insert into tbl_congresso_deficiencia_mesa (crp, cpf, nome, cracha, email, " + _
                                               ...
                                               "values (@crp, @cpf, @nome, @cracha, @email, @residencial, @comercial, @celular," + _
                                               "@titulacao, @instituicao, @recurso, @qual_recurso, @congresso, 0, 1, getdate(), 1, getdate())" + _
                                               "select * from tbl_congresso_deficiencia_mesa where id = @@identity"

  Private Shared ReadOnly C_INSERT_MESA As String = "insert into tbl_congresso_deficiencia_mesa_mesa (crp, cpf, nome, cracha, email, " + _
                                               "residencial, comercial, celular, titulacao, instituicao, recurso, qual_recurso," + _
                                               "congresso, excluido, criador, dt_criacao, atualizador, dt_atualizacao)" + _
                                               "values (@crp, @cpf, @nome, @cracha, @email, @residencial, @comercial, @celular," + _
                                               "@titulacao, @instituicao, @recurso, @qual_recurso, @congresso, 0, 1, getdate(), 1, getdate())" + _
                                               "select * from tbl_congresso_deficiencia_mesa_mesa where id = @@identity"


  Private Shared ReadOnly C_DELETE As String = "update tbl_congresso_deficiencia_mesa set excluido = 1 where id = @id"

  Private Shared ReadOnly C_UPDATE As String = "update tbl_congresso_deficiencia_mesa set crp = @crp, cpf = @cpf, nome = @nome, cracha =@cracha, email =@email," + _
                                               "residencial = @residencial, comercial = @comercial, celular =@celular, titulacao = @titulacao," + _
  ...
  Private _xestudante As Integer
...
#Region "Static Methods"
  ...
  Public Shared Function Limite() As Object
    ...
    Dim ret As New CongressoMesa
    Dim reader As SqlDataReader = SqlHelper.ExecuteReader(C_GET_DATA_ESTUDANTE)

    If (reader.Read()) Then

      ret.PopularDadosEstudante(reader)
      Return ret
...
#Region "Public Methods"
  ...
  End Sub
  Public Sub PopularDadosEstudante(ByVal reader As IDataReader)

    'popula propriedades da tabela links (criador, atualizador etc.)
    '  MyBase.PopularDados(reader)

    'popular proiedades
    Me.xestudante = reader("xestudante")
  ...
  Public Sub Inserir()
    ...
    ' executa comando no db
    Dim ret As SqlDataReader = SqlHelper.ExecuteReader(C_INSERT, _paramId, _paramCrp, _paramCpf, _paramNome, _paramCracha, _paramEmail, _paramResidencial, _paramComercial, _paramCelular, _paramTitulacao, _paramInstituicao, _paramRecurso, _paramQual_recurso, _paramCongresso)
  ...
  Public Sub InserirMesa()
    ...
    ' executa comando no db
    Dim ret As SqlDataReader = SqlHelper.ExecuteReader(C_INSERT_MESA, _paramId, _paramCrp, _paramCpf, _paramNome, _paramCracha, _paramEmail, _paramResidencial, _paramComercial, _paramCelular, _paramTitulacao, _paramInstituicao, _paramRecurso, _paramQual_recurso, _paramCongresso)
  ...
  Public Sub Alterar()
    ...
    ' executa comando no db
    SqlHelper.ExecuteNonQuery(C_UPDATE, _paramId, _paramCrp, _paramCpf, _paramNome, _paramCracha, _paramEmail, _paramResidencial, _paramComercial, _paramCelular, _paramTitulacao, _paramInstituicao, _paramRecurso, _paramQual_recurso, _paramCongresso)
...
#Region "Public Properties"
  ...
  Public Property xestudante() As Integer
    Get
      Return _xestudante
    End Get
    Set(ByVal Value As Integer)
